...
import com.MotorSport.LegendaryMotorSport.model.UserDTO;
...
public static class LoginResponse {
    ...
    @Operation(summary = "Me busca a mi mismo, evita poder buscar otros usuarios")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Usuario eliminado correctamente"),
        @ApiResponse(responseCode = "404", description = "Usuario no encontrado")
    })
    ...
    public ResponseEntity<UserDTO> getCurrentUser(@RequestHeader("Authorization") String authHeader) {
        ...
        UserDTO dto = new UserDTO(user.getUsername(), user.getEmail(), user.getLastLogin());
    ...
    public ResponseEntity<?> updateEmail(@RequestHeader("Authorization") String authHeader,
        ...
        String token = authHeader.replace("Bearer ", "");
        String oldEmail = jwtService.extractUsername(token);

        Optional<User> userOpt = userService.findByEmail(oldEmail);
        if (userOpt.isEmpty()) {
            return ResponseEntity.status(404).body("Usuario no encontrado");
        }
        ...
        return ResponseEntity.ok(new LoginResponse(user, newToken));
    }
