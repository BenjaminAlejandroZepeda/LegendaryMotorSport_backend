...
import com.MotorSport.LegendaryMotorSport.model.orderModel.OrderAdminDTO;
...
public class OrderController {
    ...
    @GetMapping
    @Operation(summary = "Obtiene todas las órdenes para el panel admin")
    ...
    public ResponseEntity<List<OrderAdminDTO>> getAllOrdersForAdmin() {
        ...
        List<OrderAdminDTO> dtos = orders.stream().map(order -> {
            String username = orderService.getUsernameByUserId(order.getUserId());
            OrderItem firstItem = order.getItems().isEmpty() ? null : order.getItems().get(0);
            String vehicleModel = firstItem != null ? firstItem.getVehicle().getModel() : "—";
            int price = order.getItems().stream().mapToInt(OrderItem::getPrecioTotal).sum();
